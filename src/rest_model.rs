...
pub struct MarginOCOOrderResult {
    ...
    pub symbol: String,
    #[serde(default, with = "as_decimal_opt")]
...
pub struct ConvertibleAssets {
    ...
    #[serde(with = "as_decimal_opt", default)]
...
pub mod as_decimal {
    ...
    where
        D: Deserializer<'de>,
    {
        // deserizlie rust_decimal from string
        let s = String::deserialize(deserializer)?;
        s.parse::<Decimal>().map_err(serde::de::Error::custom)
    }
}

pub(crate) mod as_decimal_opt {
    use std::fmt;
    ...
    {
        match value {
            Some(v) => crate::rest_model::as_decimal::serialize(v, serializer),
    ...
    where
        D: Deserializer<'de>,
    {
        Ok(Some(crate::rest_model::as_decimal::deserialize(deserializer)?))
    }
}
